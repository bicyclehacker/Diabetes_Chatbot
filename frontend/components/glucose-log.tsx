...
export function GlucoseLog() {
    ...
    const handleAddReading = async () => {
        if (!newReading.value || !newReading.type) {
            toast.error(
                'Please fill in all required fields before adding the reading.'
            );
            return;
        }

        const level = Number(newReading.value);

        // Validation range for glucose (mg/dL)
        if (isNaN(level)) {
            toast.error('Glucose level must be a valid number.');
            return;
        }

        if (level < 30) {
            toast.error('Glucose level seems too low.');
            return;
        }

        if (level > 600) {
            toast.error('Glucose level seems too high.');
            return;
        }
    ...
    const averageReading =
        readings.length > 0
            ? Math.round(
                  readings.reduce((sum, reading) => sum + reading.level, 0) /
                      readings.length
              )
            : 0;

    const trend =
        readings.length >= 2 ? readings[0].level - readings[1].level : 0;
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Track your blood glucose levels throughout the
                                day
                            </CardDescription>
                        </div>
                        <Button
                        ...
                        <div className="p-4 border rounded-lg bg-gray-50 space-y-4">
                            ...
                            <div className="flex gap-2">
                                <Button
                                    ...
                                    disabled={
                                        !newReading.value || !newReading.type
                                    }
                                >
                                    Save Reading
                                </Button>
                        ...
                        {readings.map((reading) => (
                            ...
                            >
                                <div className="flex items-center space-x-3 sm:space-x-4">
                                    <div
                                        className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${getReadingColor(
                                            reading.level,
                                            reading.readingType
                                        )}`}
                                    >
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="font-medium text-sm sm:text-base">
                                            {getTypeLabel(reading.readingType)}
                                        </p>
